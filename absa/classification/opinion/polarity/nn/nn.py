...
class NeurelNetwork(nn.Module):
    ...
    def forward(self, embed_ids: th.Tensor, graph: dgl.DGLGraph, term_mask: th.Tensor,
        ...
        h = h[term_mask.nonzero().squeeze(1)].cpu()
        target_lens = [
            int(x.sum().item()) for x in term_mask.split([l for l in sentence_len], dim=0)
        ]
